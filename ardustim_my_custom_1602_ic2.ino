...
const int latchPin = 12;    //74HC595 Pin 12 STcp
const int dataPin = 13;     //74HC595 Pin 14 DS
const int sensorPin =A0;    //LM35 attached to analog pin A0
...
void singleclick(){     
//  lcd.setCursor(0,1);// what happens when the button is clicked
...
void setDisplayNumber(int number){
  ...
  if(number < 10000 && lastRPM > 9999 || number < 1000 && lastRPM > 999){
      lcd.setCursor(4,0);
    lcd.print("         ");
  }
...
//  potvalue = map(potvalue, 0, 1023, 0, 9999);
//  potvalue = constrain(potvalue, 0, 9999);
